            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="flex-1 p-3 md:p-5 flex flex-col">
                <div className="flex-1 space-y-3 md:space-y-4">
                  ...
                  <p className="text-sm text-gray-600 line-clamp-2 mt-1">{event.about}</p>
                </div>
...
export default function TimelineEvents({ events = [], loading }: TimelineEventsProps) {
  const sortedEvents = useMemo(() => {
    if (!events || events.length === 0) return [];
    return [...events].sort((a, b) => {
      const monthA = getMonthNumber(a.month);
      const monthB = getMonthNumber(b.month);
      return monthA !== monthB ? monthA - monthB : parseInt(a.day) - parseInt(b.day);
    });
  }, [events]);
              ...
              <div className="max-w-md mx-auto">
                <div className="space-y-2 mb-4">
                  ...
                  <p className="text-sm text-gray-600">
                    Adaugă email-ul tău pentru a primi notificări despre următoarele evenimente.
                  </p>
                </div>
                <SubscribeForm />
              </div>
